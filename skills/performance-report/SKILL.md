...
> **Brand context:** Read `brand/PROFILE.md` at the plugin root before producing any output. Default voice = respectfully disrespectful (terse, ALL CAPS where it counts, occult-luxury, no filler). Brand = Vampire Sex (minimal tech house DJ duo, Miami FL, Sex Sells Records). Audience = electronic music fans, DJs, collectors, promoters.
...
3. **Data source**:
   - If Songstats is connected, discover what accounts and platforms are available, then pull performance data automatically
   - If Spotify for Artists or Apple Music for Artists is connected: pull performance data automatically
